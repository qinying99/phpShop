    ...
    <div class="logo w1210">
        ...
        <div class="user fl">
            <dl>
                ...
                <dd>
                    <div class="prompt"><?=Yii::$app->user->isGuest?'您好，欢迎来到京西！[<a href="/user/login">登录</a>]
...
<!--    开启ob缓存-->
    ...
    <div class="nav w1210 bc mt10">
        ...
        <div class="category fl <?=Yii::$app->controller->id."/".Yii::$app->controller->action->id==="index/index"?"":"cat1"?>"> <!-- 非首页，需要添加cat1类 -->
            ...
            <!--非首页加none-->
            <div class="cat_bd <?=Yii::$app->controller->id."/".Yii::$app->controller->action->id==="index/index"?"":""?>">

                <!--找出所有一级分类再循环-->
                <?php foreach (\backend\models\GoodsCategory::find()->where(['parent_id' => 0])->all() as $k1 => $v1): ?>

                    <div class="cat <?= $k1 == 0 ? "item1" : "" ?>">
                        <h3><a href="<?=\yii\helpers\Url::to(['goods/index','id'=>$v1->id])?>"><?= $v1->name ?></a> <b></b></h3>
                        <div class="cat_detail">

                            <!--通过当前分类的ID找出二级分类-->
                            <?php foreach (\backend\models\GoodsCategory::find()->where(['parent_id'=>$v1->id])->all() as $k2=>$v2): ?>

                                <dl class="<?=$k2==0?"dl_1st":""?>">
                                    <dt><a href="<?=\yii\helpers\Url::to(['goods/index','id'=>$v2->id])?>"><?=$v2->name?></a></dt>
                                    <dd>
                                        <!--通过二级分类的ID找出三级分类-->
                                        <?php foreach (\backend\models\GoodsCategory::find()->where(['parent_id'=>$v2->id])->all() as $k3=>$v3): ?>
                                            <a href="<?=\yii\helpers\Url::to(['goods/index','id'=>$v3->id])?>"><?=$v3->name?></a>
                                        <?php endforeach; ?>
                                    </dd>
                                </dl>
